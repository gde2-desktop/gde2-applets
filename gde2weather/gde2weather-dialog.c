...
struct _Gde2WeatherDialogPrivate {
...
G_DEFINE_TYPE(Gde2WeatherDialog, gde2weather_dialog, GTK_TYPE_DIALOG);

#define GDE2WEATHER_DIALOG_GET_PRIVATE(obj) \
	(G_TYPE_INSTANCE_GET_PRIVATE((obj), GDE2WEATHER_TYPE_DIALOG, Gde2WeatherDialogPrivate))

#define MONOSPACE_FONT_SCHEMA  "org.gde2.interface"
#define MONOSPACE_FONT_KEY     "monospace-font-name"

static void gde2weather_dialog_save_geometry(Gde2WeatherDialog* dialog)
{
	GSettings* settings;
	int w, h;
...
#if 0
	/* FIXME those keys are not in org.gde2.weather! */
	g_settings_set_int (settings, "dialog-width", w);
	g_settings_set_int (settings, "dialog-height", h);
#endif
}

static void gde2weather_dialog_load_geometry(Gde2WeatherDialog* dialog)
{
	GSettings* settings;
	int w, h;
...
static void response_cb(Gde2WeatherDialog* dialog, gint id, gpointer data)
{
    if (id == GTK_RESPONSE_OK) {
	gde2weather_update (dialog->priv->applet);

	gde2weather_dialog_update (dialog);
    } else {
...
static void gde2weather_dialog_create(Gde2WeatherDialog* dialog)
{
  ...
  if (gw_applet->gde2weather_pref.radar_enabled)
  ...
  gde2weather_dialog_load_geometry (dialog);
  ...
  if (gw_applet->gde2weather_pref.location->zone_valid) {
...
void gde2weather_dialog_update(Gde2WeatherDialog* dialog)
{
    ...
    if(gw_applet->gde2weather_info == NULL)
    	return;

    /* Update icon */
    icon_name = weather_info_get_icon_name (gw_applet->gde2weather_info);
    gtk_image_set_from_icon_name (GTK_IMAGE (priv->cond_image),
                                  icon_name, GTK_ICON_SIZE_DIALOG);

    /* Update current condition fields and forecast */
    gtk_label_set_text(GTK_LABEL(priv->cond_location), weather_info_get_location_name(gw_applet->gde2weather_info));
    gtk_label_set_text(GTK_LABEL(priv->cond_update), weather_info_get_update(gw_applet->gde2weather_info));
    gtk_label_set_text(GTK_LABEL(priv->cond_cond), weather_info_get_conditions(gw_applet->gde2weather_info));
    gtk_label_set_text(GTK_LABEL(priv->cond_sky), weather_info_get_sky(gw_applet->gde2weather_info));
    gtk_label_set_text(GTK_LABEL(priv->cond_temp), weather_info_get_temp(gw_applet->gde2weather_info));
    gtk_label_set_text(GTK_LABEL(priv->cond_apparent), weather_info_get_apparent(gw_applet->gde2weather_info));
    gtk_label_set_text(GTK_LABEL(priv->cond_dew), weather_info_get_dew(gw_applet->gde2weather_info));
    gtk_label_set_text(GTK_LABEL(priv->cond_humidity), weather_info_get_humidity(gw_applet->gde2weather_info));
    gtk_label_set_text(GTK_LABEL(priv->cond_wind), weather_info_get_wind(gw_applet->gde2weather_info));
    gtk_label_set_text(GTK_LABEL(priv->cond_pressure), weather_info_get_pressure(gw_applet->gde2weather_info));
    gtk_label_set_text(GTK_LABEL(priv->cond_vis), weather_info_get_visibility(gw_applet->gde2weather_info));
    gtk_label_set_text(GTK_LABEL(priv->cond_sunrise), weather_info_get_sunrise(gw_applet->gde2weather_info));
    gtk_label_set_text(GTK_LABEL(priv->cond_sunset), weather_info_get_sunset(gw_applet->gde2weather_info));
	...
	if (font_desc) {
        ...
        forecast = g_strdup(weather_info_get_forecast(gw_applet->gde2weather_info));
        if (forecast) {
	...
	radar = weather_info_get_radar (gw_applet->gde2weather_info);
        if (radar) {
...
static void gde2weather_dialog_set_property(GObject* object, guint prop_id, const GValue* value, GParamSpec* pspec)
{
    Gde2WeatherDialog *dialog = GDE2WEATHER_DIALOG (object);
...
static void gde2weather_dialog_get_property(GObject* object, guint prop_id, GValue* value, GParamSpec* pspec)
{
    Gde2WeatherDialog *dialog = GDE2WEATHER_DIALOG (object);
...
static void gde2weather_dialog_init(Gde2WeatherDialog* self)
{
    self->priv = GDE2WEATHER_DIALOG_GET_PRIVATE (self);
}

static GObject* gde2weather_dialog_constructor(GType type, guint n_construct_params, GObjectConstructParam* construct_params)
{
    GObject *object;
    Gde2WeatherDialog *self;

    object = G_OBJECT_CLASS (gde2weather_dialog_parent_class)->
        constructor (type, n_construct_params, construct_params);
    self = GDE2WEATHER_DIALOG (object);

    gde2weather_dialog_create (self);
    gde2weather_dialog_update (self);
...
{
	return g_object_new(GDE2WEATHER_TYPE_DIALOG,
		"gde2weather-applet", applet,
		NULL);
}

static void gde2weather_dialog_unrealize(GtkWidget* widget)
{
    Gde2WeatherDialog* self = GDE2WEATHER_DIALOG(widget);

    gde2weather_dialog_save_geometry(self);

    GTK_WIDGET_CLASS(gde2weather_dialog_parent_class)->unrealize(widget);
}

static void gde2weather_dialog_class_init(Gde2WeatherDialogClass* klass)
{
    GObjectClass *object_class = G_OBJECT_CLASS (klass);
    GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

    gde2weather_dialog_parent_class = g_type_class_peek_parent (klass);

    object_class->set_property = gde2weather_dialog_set_property;
    object_class->get_property = gde2weather_dialog_get_property;
    object_class->constructor = gde2weather_dialog_constructor;
    widget_class->unrealize = gde2weather_dialog_unrealize;
    ...
    g_type_class_add_private(klass, sizeof(Gde2WeatherDialogPrivate));
}
