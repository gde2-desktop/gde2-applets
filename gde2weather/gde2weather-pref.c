...
struct _Gde2WeatherPrefPrivate {
	...
	Gde2WeatherApplet* applet;
};

enum {
	PROP_0,
	PROP_GDE2WEATHER_APPLET,
};

G_DEFINE_TYPE(Gde2WeatherPref, gde2weather_pref, GTK_TYPE_DIALOG);

#define GDE2WEATHER_PREF_GET_PRIVATE(obj) \
	(G_TYPE_INSTANCE_GET_PRIVATE((obj), GDE2WEATHER_TYPE_PREF, Gde2WeatherPrefPrivate))
 ...
 * in gde2weather preference dialog.
 */
static void gde2weather_pref_set_accessibility(Gde2WeatherPref* pref)
{
...
/* Update pref dialog from gde2weather_pref */
static gboolean update_dialog(Gde2WeatherPref* pref)
{
    Gde2WeatherApplet* gw_applet = pref->priv->applet;

    g_return_val_if_fail(gw_applet->gde2weather_pref.location != NULL, FALSE);

    gtk_spin_button_set_value(GTK_SPIN_BUTTON(pref->priv->basic_update_spin), gw_applet->gde2weather_pref.update_interval / 60);
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(pref->priv->basic_update_btn), gw_applet->gde2weather_pref.update_enabled);
    soft_set_sensitive(pref->priv->basic_update_spin, gw_applet->gde2weather_pref.update_enabled);

    gtk_combo_box_set_active(GTK_COMBO_BOX(pref->priv->basic_temp_combo), gw_applet->gde2weather_pref.temperature_unit - 2);

    gtk_combo_box_set_active(GTK_COMBO_BOX(pref->priv->basic_speed_combo), gw_applet->gde2weather_pref.speed_unit - 2);

    gtk_combo_box_set_active(GTK_COMBO_BOX(pref->priv->basic_pres_combo), gw_applet->gde2weather_pref.pressure_unit - 2);

    gtk_combo_box_set_active(GTK_COMBO_BOX(pref->priv->basic_dist_combo), gw_applet->gde2weather_pref.distance_unit - 2);

	#ifdef RADARMAP
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(pref->priv->basic_radar_btn), gw_applet->gde2weather_pref.radar_enabled);

		soft_set_sensitive(pref->priv->basic_radar_url_btn, gw_applet->gde2weather_pref.radar_enabled);
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(pref->priv->basic_radar_url_btn), gw_applet->gde2weather_pref.use_custom_radar_url);
		soft_set_sensitive(pref->priv->basic_radar_url_hbox, gw_applet->gde2weather_pref.radar_enabled);

		if (gw_applet->gde2weather_pref.radar)
		{
			gtk_entry_set_text(GTK_ENTRY(pref->priv->basic_radar_url_entry), gw_applet->gde2weather_pref.radar);
		}

	#endif /* RADARMAP */

	#ifdef HAVE_LIBNOTIFY
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(pref->priv->basic_show_notifications_btn), gw_applet->gde2weather_pref.show_notifications);
	#endif
    return TRUE;
}

static void row_selected_cb(GtkTreeSelection* selection, Gde2WeatherPref* pref)
{
	Gde2WeatherApplet* gw_applet = pref->priv->applet;
	GtkTreeModel* model;
	...
	gtk_tree_model_get(model, &iter, GDE2WEATHER_XML_COL_POINTER, &loc, -1);
	...
	if (gw_applet->gde2weather_pref.location)
	{
		weather_location_free(gw_applet->gde2weather_pref.location);
	}

	gw_applet->gde2weather_pref.location = 
...
{
    Gde2WeatherPref* pref = user_data;
    WeatherLocation* loc;
    GtkTreeView* view;
    ...
    if (!weather_location_equal(loc, pref->priv->applet->gde2weather_pref.location))
    {
		return FALSE;
	}
...
static void load_locations(Gde2WeatherPref* pref)
{
	Gde2WeatherApplet* gw_applet = pref->priv->applet;
	...
	column = gtk_tree_view_column_new_with_attributes("not used", cell_renderer, "text", GDE2WEATHER_XML_COL_LOC, NULL);
	gtk_tree_view_append_column(tree, column);
	gtk_tree_view_set_expander_column(GTK_TREE_VIEW(tree), column);

	/* load locations from xml file */
	pref->priv->model = gde2weather_xml_load_locations();
	...
	if (pref->priv->applet->gde2weather_pref.location)
	{
...
static void show_notifications_toggled(GtkToggleButton* button, Gde2WeatherPref* pref)
{
	Gde2WeatherApplet* gw_applet = pref->priv->applet;
	
	gboolean toggled = gtk_toggle_button_get_active(button);
	
	if (toggled != gw_applet->gde2weather_pref.show_notifications)
	{
		/* sync with gde2weather_pref struct */
		gw_applet->gde2weather_pref.show_notifications = toggled;
...
static void auto_update_toggled(GtkToggleButton* button, Gde2WeatherPref* pref)
{
	Gde2WeatherApplet* gw_applet = pref->priv->applet;
	gboolean toggled;
	gint nxtSunEvent;
	...
	gw_applet->gde2weather_pref.update_enabled = toggled;
	...
	if (gw_applet->gde2weather_pref.update_enabled)
	{
		gw_applet->timeout_tag = g_timeout_add_seconds(gw_applet->gde2weather_pref.update_interval, timeout_cb, gw_applet);
		nxtSunEvent = weather_info_next_sun_event(gw_applet->gde2weather_info);
...
static void temp_combo_changed_cb(GtkComboBox* combo, Gde2WeatherPref* pref)
{
	Gde2WeatherApplet* gw_applet = pref->priv->applet;
	...
	old_unit = gw_applet->gde2weather_pref.temperature_unit;
	...
	gw_applet->gde2weather_pref.temperature_unit = new_unit;

	g_settings_set_enum (gw_applet->settings, "temperature-unit", new_unit);

	gtk_label_set_text(GTK_LABEL(gw_applet->label), weather_info_get_temp_summary(gw_applet->gde2weather_info));

	if (gw_applet->details_dialog)
	{
		gde2weather_dialog_update(GDE2WEATHER_DIALOG(gw_applet->details_dialog));
	}
}

static void speed_combo_changed_cb(GtkComboBox* combo, Gde2WeatherPref* pref)
{
	Gde2WeatherApplet* gw_applet = pref->priv->applet;
	...
	old_unit = gw_applet->gde2weather_pref.speed_unit;
	...
	gw_applet->gde2weather_pref.speed_unit = new_unit;
...
static void pres_combo_changed_cb(GtkComboBox* combo, Gde2WeatherPref* pref)
{
	Gde2WeatherApplet* gw_applet = pref->priv->applet;
	...
	old_unit = gw_applet->gde2weather_pref.pressure_unit;
	...
	gw_applet->gde2weather_pref.pressure_unit = new_unit;
...
static void dist_combo_changed_cb(GtkComboBox* combo, Gde2WeatherPref* pref)
{
	Gde2WeatherApplet* gw_applet = pref->priv->applet;
	...
	old_unit = gw_applet->gde2weather_pref.distance_unit;
	...
	gw_applet->gde2weather_pref.distance_unit = new_unit;
...
static void radar_toggled(GtkToggleButton* button, Gde2WeatherPref* pref)
{
    Gde2WeatherApplet* gw_applet = pref->priv->applet;
    gboolean toggled;

    toggled = gtk_toggle_button_get_active(button);
    gw_applet->gde2weather_pref.radar_enabled = toggled;
...
static void use_radar_url_toggled(GtkToggleButton* button, Gde2WeatherPref* pref)
{
    Gde2WeatherApplet* gw_applet = pref->priv->applet;
    gboolean toggled;

    toggled = gtk_toggle_button_get_active(button);
    gw_applet->gde2weather_pref.use_custom_radar_url = toggled;
    g_settings_set_boolean (gw_applet->settings, "use-custom-radar-url", toggled);
    soft_set_sensitive(pref->priv->basic_radar_url_hbox, toggled);
}

static gboolean radar_url_changed(GtkWidget* widget, GdkEventFocus* event, Gde2WeatherPref* pref)
{
	Gde2WeatherApplet* gw_applet = pref->priv->applet;
	gchar *text;
	...
	if (gw_applet->gde2weather_pref.radar)
	{
		g_free(gw_applet->gde2weather_pref.radar);
	}

	if (text)
	{
		gw_applet->gde2weather_pref.radar = g_strdup(text);
		g_free (text);
	}
	else
	{
		gw_applet->gde2weather_pref.radar = NULL;
	}

	g_settings_set_string (gw_applet->settings, "radar", gw_applet->gde2weather_pref.radar);

	return FALSE;
}

static void update_interval_changed(GtkSpinButton* button, Gde2WeatherPref* pref)
{
	Gde2WeatherApplet* gw_applet = pref->priv->applet;

	gw_applet->gde2weather_pref.update_interval = gtk_spin_button_get_value_as_int(button)*60;
	g_settings_set_int (gw_applet->settings, "auto-update-interval", gw_applet->gde2weather_pref.update_interval);
...
static gboolean free_data(GtkTreeModel* model, GtkTreePath* path, GtkTreeIter* iter, gpointer data)
{
	...
	gtk_tree_model_get(model, iter, GDE2WEATHER_XML_COL_POINTER, &location, -1);
...
static gboolean find_location(GtkTreeModel* model, GtkTreeIter* iter, const gchar* location, gboolean go_parent)
{
	...
	do {
		...
		gtk_tree_model_get (model, iter, GDE2WEATHER_XML_COL_LOC, &aux_loc, -1);
...
static void find_next_clicked(GtkButton* button, Gde2WeatherPref* pref)
{
	GtkTreeView *tree;
	GtkTreeModel *model;
	GtkEntry *entry;
...
static void find_entry_changed(GtkEditable* entry, Gde2WeatherPref* pref)
{
	GtkTreeView *tree;
	GtkTreeModel *model;
	GtkTreeSelection *selection;
...
static void help_cb(GtkDialog* dialog)
{
	...
	gtk_show_uri(gtk_widget_get_screen(GTK_WIDGET(dialog)), "help:gde2weather/gde2weather-prefs", gtk_get_current_event_time(), &error);
...
static void response_cb(GtkDialog* dialog, gint id, Gde2WeatherPref* pref)
{
...
static void gde2weather_pref_create(Gde2WeatherPref* pref)
{
	GtkWidget* pref_vbox;
   ...
   */
	...
	gde2weather_pref_set_accessibility (pref);
...
static void gde2weather_pref_set_property(GObject* object, guint prop_id, const GValue* value, GParamSpec *pspec)
{
    Gde2WeatherPref* pref = GDE2WEATHER_PREF(object);

    switch (prop_id)
    {
		case PROP_GDE2WEATHER_APPLET:
...
static void gde2weather_pref_get_property(GObject* object, guint prop_id, GValue* value, GParamSpec* pspec)
{
    Gde2WeatherPref* pref = GDE2WEATHER_PREF(object);
...
static void gde2weather_pref_init(Gde2WeatherPref* self)
{
	self->priv = GDE2WEATHER_PREF_GET_PRIVATE(self);
}


static GObject* gde2weather_pref_constructor(GType type, guint n_construct_params, GObjectConstructParam* construct_params)
{
    GObject* object;
    Gde2WeatherPref* self;

    object = G_OBJECT_CLASS(gde2weather_pref_parent_class)->constructor(type, n_construct_params, construct_params);
    self = GDE2WEATHER_PREF(object);

    gde2weather_pref_create(self);
    update_dialog(self);
...
{
    return g_object_new(GDE2WEATHER_TYPE_PREF, "gde2weather-applet", applet, NULL);
}


static void gde2weather_pref_finalize(GObject* object)
{
	Gde2WeatherPref* self = GDE2WEATHER_PREF(object);

	gtk_tree_model_foreach(self->priv->model, free_data, NULL);
	g_object_unref(G_OBJECT(self->priv->model));

	G_OBJECT_CLASS(gde2weather_pref_parent_class)->finalize(object);
}


static void gde2weather_pref_class_init(Gde2WeatherPrefClass* klass)
{
    GObjectClass* object_class = G_OBJECT_CLASS(klass);

    gde2weather_pref_parent_class = g_type_class_peek_parent(klass);

    object_class->set_property = gde2weather_pref_set_property;
    object_class->get_property = gde2weather_pref_get_property;
    object_class->constructor = gde2weather_pref_constructor;
    object_class->finalize = gde2weather_pref_finalize;

    /* This becomes an OBJECT property when Gde2WeatherApplet is redone */
    g_object_class_install_property(object_class, PROP_GDE2WEATHER_APPLET, g_param_spec_pointer("gde2weather-applet", "Gde2Weather Applet", "The Gde2Weather Applet", G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

    g_type_class_add_private(klass, sizeof(Gde2WeatherPrefPrivate));
}
