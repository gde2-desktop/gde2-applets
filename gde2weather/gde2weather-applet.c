		      ...
		      Gde2WeatherApplet *gw_applet)
{
    ...
    gde2weather_about_run (gw_applet);
}
		     ...
		     Gde2WeatherApplet *gw_applet)
{
   if (gw_applet->pref_dialog) {
	...
	gw_applet->pref_dialog = gde2weather_pref_new(gw_applet);
			...
			Gde2WeatherApplet *gw_applet)
{
   if (gw_applet->details_dialog) {
	...
	gw_applet->details_dialog = gde2weather_dialog_new(gw_applet);
...
static void place_widgets (Gde2WeatherApplet *gw_applet)
{
    GtkRequisition req;
    int total_size = 0;
    ...
    temp = weather_info_get_temp_summary(gw_applet->gde2weather_info);
    if (temp) 
...
static void change_orient_cb (Gde2PanelApplet *w, Gde2PanelAppletOrient o, gpointer data)
{
    Gde2WeatherApplet *gw_applet = (Gde2WeatherApplet *)data;
...
static void size_allocate_cb(Gde2PanelApplet *w, GtkAllocation *allocation, gpointer data)
{
    Gde2WeatherApplet *gw_applet = (Gde2WeatherApplet *)data;
	
    if ((gw_applet->orient == GDE2_PANEL_APPLET_ORIENT_LEFT) || (gw_applet->orient == GDE2_PANEL_APPLET_ORIENT_RIGHT)) {
...
{
    Gde2WeatherApplet *gw_applet = data;
...
static gboolean 
key_press_cb (GtkWidget *widget, GdkEventKey *event, Gde2WeatherApplet *gw_applet)
{
	switch (event->keyval) {	
	case GDK_KEY_u:
...
static void
network_changed (GNetworkMonitor *monitor, gboolean available, Gde2WeatherApplet *gw_applet)
{
    if (available) {
        gde2weather_update (gw_applet);
    }
...
static void
applet_destroy (GtkWidget *widget, Gde2WeatherApplet *gw_applet)
{
    ...
    weather_info_abort (gw_applet->gde2weather_info);
}

void gde2weather_applet_create (Gde2WeatherApplet *gw_applet)
{
    ...
    gw_applet->gde2weather_pref.location = NULL;
    gw_applet->gde2weather_pref.show_notifications = FALSE;
    gw_applet->gde2weather_pref.update_interval = 1800;
    gw_applet->gde2weather_pref.update_enabled = TRUE;
    gw_applet->gde2weather_pref.detailed = FALSE;
    gw_applet->gde2weather_pref.radar_enabled = TRUE;
    gw_applet->gde2weather_pref.temperature_unit = TEMP_UNIT_INVALID;
    gw_applet->gde2weather_pref.speed_unit = SPEED_UNIT_INVALID;
    gw_applet->gde2weather_pref.pressure_unit = PRESSURE_UNIT_INVALID;
    gw_applet->gde2weather_pref.distance_unit = DISTANCE_UNIT_INVALID;
    
    gde2_panel_applet_set_flags (gw_applet->applet, GDE2_PANEL_APPLET_EXPAND_MINOR);

    gde2_panel_applet_set_background_widget(gw_applet->applet,
    ...
    gtk_widget_set_tooltip_text (GTK_WIDGET(gw_applet->applet), _("GDE2 Weather"));

    atk_obj = gtk_widget_get_accessible (GTK_WIDGET (gw_applet->applet));
    if (GTK_IS_ACCESSIBLE (atk_obj))
	   atk_object_set_name (atk_obj, _("GDE2 Weather"));

    gw_applet->size = gde2_panel_applet_get_size (gw_applet->applet);

    gw_applet->orient = gde2_panel_applet_get_orient (gw_applet->applet);

    action_group = gtk_action_group_new ("Gde2Weather Applet Actions");
    gtk_action_group_set_translation_domain (action_group, GETTEXT_PACKAGE);
    gtk_action_group_add_actions (action_group,
				  weather_applet_menu_actions,
				  G_N_ELEMENTS (weather_applet_menu_actions),
				  gw_applet);
    ui_path = g_build_filename (GDE2WEATHER_MENU_UI_DIR, "gde2weather-applet-menu.xml", NULL);
    gde2_panel_applet_setup_menu_from_file (gw_applet->applet,
				       ui_path, action_group);
    g_free (ui_path);

    if (gde2_panel_applet_get_locked_down (gw_applet->applet)) {
	    GtkAction *action;
...
gint timeout_cb (gpointer data)
{
    ...
    gde2weather_update(gw_applet);
    return 0;  /* Do not repeat timeout (will be reset by gde2weather_update) */
}
...
update_finish (WeatherInfo *info, gpointer data)
{
	...
	gw_applet->timeout_tag =
		g_timeout_add_seconds (
                       gw_applet->gde2weather_pref.update_interval,
	     ...
	     (gw_fault_counter >= MAX_CONSECUTIVE_FAULTS))
    {
	    ...
	    gtk_label_set_text (GTK_LABEL (gw_applet->label), 
					...
					gw_applet->gde2weather_info));
	    
	    s = weather_info_get_weather_summary (gw_applet->gde2weather_info);
...
#ifdef HAVE_LIBNOTIFY
        if (gw_applet->gde2weather_pref.show_notifications)
        {
		    ...
		    if (notify_is_initted ())
		    {
			 ...
			 icon = weather_info_get_icon_name (gw_applet->gde2weather_info);
			 if (icon == NULL)
				 icon = "stock-unknown";
...
{
    WeatherInfo *info = ((Gde2WeatherApplet *)data)->gde2weather_info;
    update_finish(info, data);
    return 0;  /* Do not repeat timeout (will be reset by update_finish) */
}


void gde2weather_update (Gde2WeatherApplet *gw_applet)
{
    WeatherPrefs prefs;
    const gchar *icon_name;
    ...
    prefs.type = gw_applet->gde2weather_pref.detailed ? FORECAST_ZONE : FORECAST_STATE;

    /* Set radar map retrieval option */
    prefs.radar = gw_applet->gde2weather_pref.radar_enabled;
    prefs.radar_custom_url = (gw_applet->gde2weather_pref.use_custom_radar_url &&
    				gw_applet->gde2weather_pref.radar) ?
				gw_applet->gde2weather_pref.radar : NULL;

    /* Set the units */
    prefs.temperature_unit = gw_applet->gde2weather_pref.temperature_unit;
    prefs.speed_unit = gw_applet->gde2weather_pref.speed_unit;
    prefs.pressure_unit = gw_applet->gde2weather_pref.pressure_unit;
    prefs.distance_unit = gw_applet->gde2weather_pref.distance_unit;

    /* Update current conditions */
    if (gw_applet->gde2weather_info && 
    	weather_location_equal(weather_info_get_location(gw_applet->gde2weather_info),
    			       gw_applet->gde2weather_pref.location)) {
	weather_info_update(gw_applet->gde2weather_info, &prefs,
			    update_finish, gw_applet);
    } else {
        weather_info_free(gw_applet->gde2weather_info);
        gw_applet->gde2weather_info = weather_info_new(gw_applet->gde2weather_pref.location,
						    &prefs,
						    update_finish, gw_applet);
    }
}
