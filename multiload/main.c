...
start_procman (MultiloadApplet *ma)
{
	...
	if (monitor == NULL)
	        monitor = g_strdup ("gde2-system-monitor.desktop");
	...
	if (appinfo) {
		...
		app_info = g_app_info_create_from_commandline ("gde2-system-monitor",
...
static void
multiload_change_size_cb(Gde2PanelApplet *applet, gint size, gpointer data)
{
...
static void
multiload_change_orient_cb(Gde2PanelApplet *applet, gint arg1, gpointer data)
{
...
multiload_applet_refresh(MultiloadApplet *ma)
{
	...
	orientation = gde2_panel_applet_get_orient(ma->applet);
...
static gboolean
multiload_applet_new(Gde2PanelApplet *applet, const gchar *iid, gpointer data)
{
	MultiloadApplet *ma;
	...
	ma->settings = gde2_panel_applet_settings_new (applet, "org.gde2.panel.applet.multiload");
	...
	lockdown_settings = g_settings_new ("org.gde2.lockdown");
	if (g_settings_get_boolean (lockdown_settings, "disable-command-line") ||
	    gde2_panel_applet_get_locked_down (applet)) {
		GtkAction *action;
...
static gboolean
multiload_factory (Gde2PanelApplet *applet,
				const gchar *iid,
				gpointer data)
{
...
GDE2_PANEL_APPLET_OUT_PROCESS_FACTORY ("MultiLoadAppletFactory",
				  PANEL_TYPE_APPLET,
				  "multiload",
				  multiload_factory,
				  NULL)
