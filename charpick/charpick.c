/* charpick.c  This is a gde2 panel applet that allow users to select
...
{
	gchar *string;
	Gde2PanelApplet *applet = GDE2_PANEL_APPLET (curr_data->applet);
...
get_menu_pos (GtkMenu *menu, gint *x, gint *y, gboolean *push_in, gpointer data)
{
	...
	gdk_window_get_geometry (GDK_WINDOW (gtk_widget_get_window(curr_data->applet)), NULL, NULL,
     	...
     	switch (gde2_panel_applet_get_orient (GDE2_PANEL_APPLET (curr_data->applet))) {
...
build_table(charpick_data *p_curr_data)
{
  ...
  if (g_list_length (p_curr_data->chartable) != 1)
  {
    ...
    switch (gde2_panel_applet_get_orient (GDE2_PANEL_APPLET (p_curr_data->applet))) {
       	case GDE2_PANEL_APPLET_ORIENT_DOWN:
#if GTK_CHECK_VERSION (3, 0, 0)
...
static void applet_size_allocate(Gde2PanelApplet *applet, GtkAllocation *allocation, gpointer data)
{
  charpick_data *curr_data = data;
...
static void applet_change_orient(Gde2PanelApplet *applet, Gde2PanelAppletOrient o, gpointer data)
{
  charpick_data *curr_data = data;
  if (o == GDE2_PANEL_APPLET_ORIENT_UP ||
      o == GDE2_PANEL_APPLET_ORIENT_DOWN)
       ...
       charpick_data *curr_data)
{
  ...
  gde2_show_about_dialog (NULL,
	...
	"copyright",	"\xC2\xA9 1998, 2004-2005 GNOME Applets Maintainers "
			"and others",
	"comments",	_("Gde2 Panel applet for selecting strange "
...
{
	gde2_panel_applet_settings_set_glist (curr_data->settings,
...
get_chartable (charpick_data *curr_data)
{
	...
	value = gde2_panel_applet_settings_get_glist (curr_data->settings, "chartable");
	if (value) {
...
static gboolean
charpicker_applet_fill (Gde2PanelApplet *applet)
{
  ...
  curr_data->settings = gde2_panel_applet_settings_new (applet, "org.gde2.panel.applet.charpick");
  ...
  curr_data->panel_size = gde2_panel_applet_get_size (applet);
  
  orientation = gde2_panel_applet_get_orient (applet);
  curr_data->panel_vertical = (orientation == GDE2_PANEL_APPLET_ORIENT_LEFT) 
                              || (orientation == GDE2_PANEL_APPLET_ORIENT_RIGHT);
  build_table (curr_data);
...
static gboolean
charpicker_applet_factory (Gde2PanelApplet *applet,
			   const gchar          *iid,
			   gpointer              data)
{
...
GDE2_PANEL_APPLET_OUT_PROCESS_FACTORY ("CharpickerAppletFactory",
				  PANEL_TYPE_APPLET,
				  "char-palette",
