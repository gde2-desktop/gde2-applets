...
selector_is_available (void)
{
	...
	if (!system_bus) {
        ...
        proxy = dbus_g_proxy_new_for_name (system_bus,
                                           "org.gde2.CPUFreqSelector",
                                           "/org/gde2/cpufreq_selector/selector",
                                           "org.gde2.CPUFreqSelector");
        ...
        if (!dbus_g_proxy_call (proxy, "CanSet", &error,
                           	...
                           	G_TYPE_INVALID)) {
		g_warning ("Error calling org.gde2.CPUFreqSelector.CanSet: %s", error->message);
		g_error_free (error);
		result = FALSE;
	}
