...
import gde2_invest, gde2_invest.about, gde2_invest.chart, gde2_invest.preferences
from gde2_invest.quotes import QuoteUpdater
from gde2_invest.widgets import *

Gtk.Window.set_default_icon_from_file(join(gde2_invest.ART_DATA_DIR, "invest_neutral.svg"))

class InvestApplet(Gde2PanelApplet.Applet):
	def __init__(self, applet):
		...
		actiongroup.set_translation_domain(gde2_invest.defs.GETTEXT_PACKAGE)
		actiongroup.add_actions(menu_actions, None)
		self.applet.setup_menu_from_file (join(gde2_invest.defs.PKGDATADIR, "Invest_Applet.xml"), actiongroup)
	...
	def button_clicked(self, widget,event):
		if event.type == Gdk.EventType.BUTTON_PRESS and event.button == 1:
			...
			if len (gde2_invest.STOCKS) == 0:
				...
				gde2_invest.preferences.show_preferences(self, _("<b>You have not entered any stock information yet</b>"))
				self.reload_ilw()
	...
	def on_about(self, action):
		gde2_invest.about.show_about()
	
	def on_help(self, action):
		gde2_invest.help.show_help()

	def on_preferences(self, action):
		gde2_invest.preferences.show_preferences(self)
		self.reload_ilw()
	...
	def set_applet_icon(self, change):
		if change == 1:
			pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_size(join(gde2_invest.ART_DATA_DIR, "invest-22_up.png"), -1,-1)
		elif change == 0:
			pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_size(join(gde2_invest.ART_DATA_DIR, "invest-22_neutral.png"), -1,-1)
		else:
			pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_size(join(gde2_invest.ART_DATA_DIR, "invest-22_down.png"), -1,-1)
...
class InvestmentsListWindow(Gtk.Window):
	...
	def update_position (self):
		...
		if self.alignment == Gde2PanelApplet.AppletOrient.LEFT:
			x = ax - ww
			y = ay
		...
		elif self.alignment == Gde2PanelApplet.AppletOrient.RIGHT:
			x = ax + aw
			y = ay
		...
		elif self.alignment == Gde2PanelApplet.AppletOrient.DOWN:
			x = ax
			y = ay + ah
			...
			gravity = Gdk.Gravity.NORTH_WEST
		elif self.alignment == Gde2PanelApplet.AppletOrient.UP:
			x = ax
			y = ay - wh
