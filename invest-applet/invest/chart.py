...
class ImageRetriever(Thread, _IdleObject):
	...
	def run(self):
		...
		try: sock = urllib.urlopen(self.image_url, proxies = gde2_invest.PROXY)
		except Exception, msg:
			gde2_invest.debug("Error while opening %s: %s" % (self.image_url, msg))
		else:
...
class FinancialChart:
	def __init__(self, ui):
		...
		try:
			pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_size(join(gde2_invest.ART_DATA_DIR, "invest_neutral.svg"), 96,96)
			self.ui.get_object("plot").set_from_pixbuf(pixbuf)
		except Exception, msg:
			gde2_invest.debug("Could not load 'invest-neutral.svg' file: %s" % msg)
			pass
...
def show_chart(tickers):
	ui = Gtk.Builder();
	ui.add_from_file(os.path.join(gde2_invest.BUILDER_DATA_DIR, "financialchart.ui"))
