...
import gde2_invest, gde2_invest.about, gde2_invest.chart
import currencies
...
class QuotesRetriever(Thread, _IdleObject):
	...
	def __init__(self, tickers):
		...
		self.data = []
		self.currencies = []
                gde2_invest.debug("QuotesRetriever created");

	def run(self):
                gde2_invest.debug("QuotesRetriever started");
		quotes_url = QUOTES_URL % {"s": self.tickers}
		try:
			quotes_file = urlopen(quotes_url, proxies = gde2_invest.PROXY)
			self.data = quotes_file.read ()
			quotes_file.close ()
		except Exception, msg:
			gde2_invest.debug("Error while retrieving quotes data (url = %s): %s" % (quotes_url, msg))
		else:
			self.retrieved = True
		self.emit("completed")
...
class QuoteUpdater(Gtk.ListStore):
	...
	def __init__ (self, change_icon_callback, set_tooltip_callback):
		...
		gde2_invest.nm.set_statechange_callback(self.nm_state_changed)

	# loads the cached csv file and its last-modification-time as self.last_updated
	def load_quotes(self):
		gde2_invest.debug("Loading quotes");
		try:
			f = open(gde2_invest.QUOTES_FILE, 'r')
			data = f.readlines()
			f.close()

			self.populate(self.parse_yahoo_csv(csv.reader(data)))
			self.updated = True
			self.last_updated = datetime.datetime.fromtimestamp(getmtime(gde2_invest.QUOTES_FILE))
			self.update_tooltip()
		except Exception, msg:
			gde2_invest.error("Could not load the cached quotes file %s: %s" % (gde2_invest.QUOTES_FILE, msg) )

	# stores the csv content on disk so it can be used on next start up
	def save_quotes(self, data):
		gde2_invest.debug("Storing quotes")
		try:
			f = open(gde2_invest.QUOTES_FILE, 'w')
			f.write(data)
			f.close()
		except Exception, msg:
			gde2_invest.error("Could not save the retrieved quotes file to %s: %s" % (gde2_invest.QUOTES_FILE, msg) )

	def set_update_interval(self, interval):
		if self.timeout_id != None:
			gde2_invest.debug("Canceling refresh timer")
			GObject.source_remove(self.timeout_id)
			self.timeout_id = None
		if interval > 0:
			gde2_invest.debug("Setting refresh timer to %s:%02d.%03d" % ( interval / 60000, interval % 60000 / 1000, interval % 1000) )
	...
	def nm_state_changed(self):
		...
		if gde2_invest.nm.online():
			if self.timeout_id == None:
				self.set_update_interval(AUTOREFRESH_TIMEOUT)
				self.refresh()

	def refresh(self):
		gde2_invest.debug("Refreshing")

		# when nm tells me I am offline, stop the update interval
		if gde2_invest.nm.offline():
			gde2_invest.debug("We are offline, stopping update timer")
			self.set_update_interval(0)
			return False

		if len(gde2_invest.STOCKS) == 0:
                        gde2_invest.debug("No stocks configured")
			return True

		tickers = '+'.join(gde2_invest.STOCKS.keys())
                gde2_invest.debug("creating QuotesRetriever")
		quotes_retriever = QuotesRetriever(tickers)
		quotes_retriever.connect("completed", self.on_retriever_completed)
                gde2_invest.debug("starting QuotesRetriever")
		quotes_retriever.start()
                gde2_invest.debug("started QuotesRetriever")
	...
	def on_retriever_completed(self, retriever):
		if retriever.retrieved == False:
                        gde2_invest.debug("QuotesRetriever failed");
                        self.update_tooltip(_('Invest could not connect to Yahoo! Finance'))

		else:
                        gde2_invest.debug("QuotesRetriever completed");
	...
	def on_currency_retriever_completed(self, retriever):
		if retriever.retrieved == False:
			gde2_invest.error("Failed to retrieve currency rates!")
		else:
			gde2_invest.debug("CurrencyRetriever completed")
			self.save_currencies(retriever.data)
			self.load_currencies()
		self.update_tooltip()

	def save_currencies(self, data):
		gde2_invest.debug("Storing currencies to %s" % gde2_invest.CURRENCIES_FILE)
		try:
			f = open(gde2_invest.CURRENCIES_FILE, 'w')
			f.write(data)
			f.close()
		except Exception as msg:
			gde2_invest.error("Could not save the retrieved currencies to %s: %s" % (gde2_invest.CURRENCIES_FILE, msg) )

	def load_currencies(self):
		gde2_invest.debug("Loading currencies from %s" % gde2_invest.CURRENCIES_FILE)
		try:
			f = open(gde2_invest.CURRENCIES_FILE, 'r')
			data = f.readlines()
			f.close()

			self.convert_currencies(self.parse_yahoo_csv(csv.reader(data)))
		except Exception as msg:
			gde2_invest.error("Could not load the currencies from %s: %s" % (gde2_invest.CURRENCIES_FILE, msg) )
		...
		try:
			...
			for ticker, val in quote_items:
				pb = None
                                ...
                                if ticker not in gde2_invest.STOCKS.keys():
                                    gde2_invest.debug("Observed unknown stock: %s" % ticker)
                                    continue

				# get the label of this stock for later reuse
				label = gde2_invest.STOCKS[ticker]["label"]
				if len(label) == 0:
					if len(val["label"]) != 0:
						label = val["label"]
					else:
				...
				if len(val["currency"]) > 0 and val["currency"] not in currencies.Currencies.currencies:
					gde2_invest.debug("Currency '%s' is not known, dropping" % val["currency"])
					val["currency"] = ""
				...
				for purchase in gde2_invest.STOCKS[ticker]["purchases"]:
					if purchase["amount"] != 0:
				...
				if is_simple_quote:
					...
					(balance, change) = self.balance(gde2_invest.STOCKS[ticker]["purchases"], val["trade"])
				...
				image_retriever = gde2_invest.chart.ImageRetriever(url)
			...
			gde2_invest.debug("Failed to populate quotes: %s" % msg)
			gde2_invest.debug(quotes)
			self.quotes_valid = False

		# start retrieving currency conversion rates
		if gde2_invest.CONFIG.has_key("currency"):
			target_currency = gde2_invest.CONFIG["currency"]
			symbols = []

			gde2_invest.debug("These currencies occur: %s" % self.currencies)
			for currency in self.currencies:
				if currency == target_currency:
					continue

				gde2_invest.debug("%s will be converted to %s" % ( currency, target_currency ))
	...
	def convert_currencies(self, quotes):
		...
		if not gde2_invest.CONFIG.has_key("currency"):
			return
			...
			if not ( len(symbol) == 8 and symbol[6:8] == "=X" ) and \
			   currency != target_currency and rates.has_key(currency):
				...
				if not self.get_value(iter, self.TICKER_ONLY):
					...
					(balance, change) = self.balance(gde2_invest.STOCKS[ticker]["purchases"], value, rates[currency])
	...
	def simple_quotes_only(self):
		res = True
		for entry, data in gde2_invest.STOCKS.iteritems():
