...
#		self.set_property("reorderable", True)
	...
	def on_row_activated(self, treeview, path, view_column):
		...
		gde2_invest.chart.show_chart([ticker])
...
class InvestTrend(Gtk.Image):
	...
	def set_color(self, color, opacity=0xFF):
		if self.pixbuf != None:
			try:
				...
				gde2_invest.error("Could not set color: %s" % msg)
	...
	def on_quotes_update(self, updater):
		...
		for row in updater:
			...
			portfolio_number = sum([purchase["amount"] for purchase in gde2_invest.STOCKS[row[updater.SYMBOL]]["purchases"]])
