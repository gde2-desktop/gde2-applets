 ...
 *
 *  Gde2 Netspeed Applet migrated by Stefano Karapetsas <stefano@karapetsas.com>
 */
...
static const char* const dev_type_icon[DEV_UNKNOWN + 1] = {
	"gde2-netspeed-loopback",    /* DEV_LO */
	"network-wired",             /* DEV_ETHERNET */
	"network-wireless",          /* DEV_WIRELESS */
	"gde2-netspeed-ppp",         /* DEV_PPP */
	"gde2-netspeed-plip",        /* DEV_PLIP */
	"gde2-netspeed-plip",        /* DEV_SLIP */
	"network-workgroup",         /* DEV_UNKNOWN */
};

static const char* wireless_quality_icon[] = {
	"gde2-netspeed-wireless-25",
	"gde2-netspeed-wireless-50",
	"gde2-netspeed-wireless-75",
	"gde2-netspeed-wireless-100"
};

static const char IN_ICON[] = "go-down";
static const char OUT_ICON[] = "go-up";
static const char ERROR_ICON[] = "gtk-dialog-error";
static const char LOGO_ICON[] = "gde2-netspeed-applet";
...
static void
update_tooltip(Gde2NetspeedApplet* applet);

static void
device_change_cb(GtkComboBox *combo, Gde2NetspeedApplet *applet);
 ...
 */
static void
applet_change_size_or_orient(Gde2PanelApplet *applet_widget, int arg1, Gde2NetspeedApplet *applet)
{
	int size;
	...
	size = gde2_panel_applet_get_size(applet_widget);
	orient = gde2_panel_applet_get_orient(applet_widget);
 ...
 */
static void
change_icons(Gde2NetspeedApplet *applet)
{
	GdkPixbuf *dev, *down;
 ...
 */
static void
change_quality_icon(Gde2NetspeedApplet *applet)
{
...
static void
update_quality_icon(Gde2NetspeedApplet *applet)
{
...
static void
init_quality_pixbufs(Gde2NetspeedApplet *applet)
{
	GtkIconTheme *icon_theme;
	int i;
	GdkPixbuf *pixbuf;
...
{
    Gde2NetspeedApplet *applet = (Gde2NetspeedApplet*)user_data;
 ...
 */
static void
redraw_graph(Gde2NetspeedApplet *applet, cairo_t *cr)
{
...
static gboolean
set_applet_devinfo(Gde2NetspeedApplet* applet, const char* iface)
{
	DevInfo info;
...
static void
search_for_up_if(Gde2NetspeedApplet *applet)
{
...
static void
update_applet(Gde2NetspeedApplet *applet)
{
...
static gboolean
timeout_function(Gde2NetspeedApplet *applet)
{
	if (!applet)
		return FALSE;
...
display_help (GtkWidget *dialog, const gchar *section)
{
	...
	if (section)
		uri = g_strdup_printf ("help:gde2-netspeed-applet/%s", section);
	else
		uri = g_strdup ("help:gde2-netspeed-applet");
 ...
 */
static void
help_cb (GtkAction *action, Gde2NetspeedApplet *ap)
{
...
{
    Gde2NetspeedApplet *applet = data;
 ...
 */
static void
showsum_change_cb(GtkToggleButton *togglebutton, Gde2NetspeedApplet *applet)
{
 ...
 */
static void
showbits_change_cb(GtkToggleButton *togglebutton, Gde2NetspeedApplet *applet)
{
 ...
 */
static void
shortunit_change_cb(GtkToggleButton *togglebutton, Gde2NetspeedApplet *applet)
{
 ...
 */
static void
showicon_change_cb(GtkToggleButton *togglebutton, Gde2NetspeedApplet *applet)
{
 ...
 */
static void
showqualityicon_change_cb(GtkToggleButton *togglebutton, Gde2NetspeedApplet *applet)
{
 ...
 */
static void
changeicon_change_cb(GtkToggleButton *togglebutton, Gde2NetspeedApplet *applet)
{
...
{
	Gde2NetspeedApplet *applet = (Gde2NetspeedApplet*)data;
	GtkWidget *vbox;
	GtkWidget *hbox;
	...
	applet->settings = GTK_DIALOG(gtk_dialog_new_with_buttons(_("Gde2 Netspeed Preferences"),
								  NULL,
...
static void
incolor_changed_cb (Gde2ColorButton *cb, gpointer data)
{
	Gde2NetspeedApplet *applet = (Gde2NetspeedApplet*)data;
	gchar *color;
	GdkColor clr;

	gde2_color_button_get_color (cb, &clr);
	applet->in_color = clr;
...
static void
outcolor_changed_cb (Gde2ColorButton *cb, gpointer data)
{
	Gde2NetspeedApplet *applet = (Gde2NetspeedApplet*)data;
	gchar *color;
	GdkColor clr;
 ...
 */
static void
info_response_cb (GtkDialog *dialog, gint id, Gde2NetspeedApplet *applet)
{
...
showinfo_cb(GtkAction *action, gpointer data)
{
	...
	incolor_sel =  gde2_color_button_new ();
	outcolor_sel = gde2_color_button_new ();

	gde2_color_button_set_color (GDE2_COLOR_BUTTON (incolor_sel),  &applet->in_color);
	gde2_color_button_set_color (GDE2_COLOR_BUTTON (outcolor_sel),  &applet->out_color);
...
static const GtkActionEntry gde2_netspeed_applet_menu_actions [] = {
		{ "Gde2NetspeedAppletDetails", GTK_STOCK_INFO, N_("Device _Details"),
		  NULL, NULL, G_CALLBACK (showinfo_cb) },
		{ "Gde2NetspeedAppletProperties", GTK_STOCK_PROPERTIES, N_("Preferences..."),
		  NULL, NULL, G_CALLBACK (settings_cb) },
		{ "Gde2NetspeedAppletHelp", GTK_STOCK_HELP, N_("Help"),
		  NULL, NULL, G_CALLBACK (help_cb) },
		{ "Gde2NetspeedAppletAbout", GTK_STOCK_ABOUT, N_("About..."),
 ...
 * "jumping around" in the gde2_panel which looks uggly
 */
static void
label_size_request_cb(GtkWidget *widget, GtkRequisition *requisition, Gde2NetspeedApplet *applet)
{
...
static gboolean
applet_button_press(GtkWidget *widget, GdkEventButton *event, Gde2NetspeedApplet *applet)
{
	if (event->button == 1)
	{
		GError *error = NULL;
 ...
 */
static void
applet_destroy(Gde2PanelApplet *applet_widget, Gde2NetspeedApplet *applet)
{
	GtkIconTheme *icon_theme;
...
static gboolean
gde2_netspeed_enter_cb(GtkWidget *widget, GdkEventCrossing *event, gpointer data)
{
	Gde2NetspeedApplet *applet = data;
...
static gboolean
gde2_netspeed_leave_cb(GtkWidget *widget, GdkEventCrossing *event, gpointer data)
{
	Gde2NetspeedApplet *applet = data;
 ...
 */
static gboolean
gde2_netspeed_applet_factory(Gde2PanelApplet *applet_widget, const gchar *iid, gpointer data)
{
	Gde2NetspeedApplet *applet;
	int i;
	char* menu_string;
	GtkIconTheme *icon_theme;
	...
	gde2_panel_applet_set_background_widget(GDE2_PANEL_APPLET(applet_widget),
	...
	glibtop_init();
	g_set_application_name (_("Gde2 Netspeed"));
	 ...
	 */
	applet = g_malloc0(sizeof(Gde2NetspeedApplet));
	...
	*/
	...
	applet->gsettings = gde2_panel_applet_settings_new (applet_widget, "org.gde2.panel.applet.netspeed");
	...
	gde2_panel_applet_set_flags(applet_widget, GDE2_PANEL_APPLET_EXPAND_MINOR);
	...
	g_signal_connect(G_OBJECT(applet_widget), "leave_notify_event",
			 G_CALLBACK(gde2_netspeed_leave_cb),
			 (gpointer)applet);

	g_signal_connect(G_OBJECT(applet_widget), "enter_notify_event",
			 G_CALLBACK(gde2_netspeed_enter_cb),
			 (gpointer)applet);
	...
	gtk_action_group_add_actions (action_group,
                                  gde2_netspeed_applet_menu_actions,
                                  G_N_ELEMENTS (gde2_netspeed_applet_menu_actions),
                                  applet);
	ui_path = g_build_filename (NETSPEED_MENU_UI_DIR, "netspeed-menu.xml", NULL);
	gde2_panel_applet_setup_menu_from_file (GDE2_PANEL_APPLET (applet->applet), ui_path, action_group);
	g_free (ui_path);
...
GDE2_PANEL_APPLET_OUT_PROCESS_FACTORY("NetspeedAppletFactory",
									  PANEL_TYPE_APPLET,
									  "NetspeedApplet",
									  gde2_netspeed_applet_factory,
									  NULL)
