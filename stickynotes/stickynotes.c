...
response_cb (GtkWidget *dialog, gint id, gpointer data)
{
		...
		gtk_show_uri (gtk_widget_get_screen (GTK_WIDGET (dialog)),
				"help:gde2-stickynotes-applet/stickynotes-settings-individual",
...
stickynotes_save_now (void)
{
	...
	/* The XML file is $HOME/.config/gde2/stickynotes-applet, most probably */
	{
			gchar* file = g_build_filename(g_get_user_config_dir(), "gde2", "stickynotes-applet.xml", NULL);
...
stickynotes_load (GdkScreen *screen)
{
	...
	/* The XML file is $HOME/.config/gde2/stickynotes-applet, most probably */
	{
		...
		else
		{
			...
			file = g_build_filename(g_get_home_dir(), ".gde22", "stickynotes_applet", NULL);
