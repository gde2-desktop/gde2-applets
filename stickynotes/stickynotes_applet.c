...
static gboolean stickynotes_applet_factory(Gde2PanelApplet *gde2_panel_applet, const gchar *iid, gpointer data)
{
	if (!strcmp(iid, "StickyNotesApplet")) {
		if (!stickynotes)
			stickynotes_applet_init (gde2_panel_applet);

		gde2_panel_applet_set_flags (gde2_panel_applet, GDE2_PANEL_APPLET_EXPAND_MINOR);

		/* Add applet to linked list of all applets */
		stickynotes->applets = g_list_append (stickynotes->applets, stickynotes_applet_new(gde2_panel_applet));
...
/* Sticky Notes applet factory */
GDE2_PANEL_APPLET_OUT_PROCESS_FACTORY("StickyNotesAppletFactory", PANEL_TYPE_APPLET, "stickynotes_applet",
...
void
stickynotes_applet_init (Gde2PanelApplet *gde2_panel_applet)
{
	...
	gtk_window_set_default_icon_name ("gde2-sticky-notes-applet");

	stickynotes->icon_normal = gtk_icon_theme_load_icon (
			gtk_icon_theme_get_default (),
			"gde2-sticky-notes-applet",
			48, 0, NULL);
	...
	/* Load sticky notes */
	stickynotes_load (gtk_widget_get_screen (GTK_WIDGET (gde2_panel_applet)));
...
StickyNotesApplet *
stickynotes_applet_new(Gde2PanelApplet *gde2_panel_applet)
{
	AtkObject *atk_obj;
	gchar *ui_path;
	...
	applet->w_applet = GTK_WIDGET(gde2_panel_applet);
	...
	/* Add the applet icon */
	gtk_container_add(GTK_CONTAINER(gde2_panel_applet), applet->w_image);
	applet->panel_size = gde2_panel_applet_get_size (gde2_panel_applet);
	applet->panel_orient = gde2_panel_applet_get_orient (gde2_panel_applet);
	...
	gde2_panel_applet_setup_menu_from_file(gde2_panel_applet, ui_path, applet->action_group);
	g_free (ui_path);

	if (gde2_panel_applet_get_locked_down (gde2_panel_applet)) {
		GtkAction *action;
	...
	gde2_panel_applet_set_background_widget (gde2_panel_applet, applet->w_applet);
