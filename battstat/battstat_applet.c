/* battstat        A GDE2 battery meter for laptops.
...
#define BATTSTAT_SCHEMA "org.gde2.panel.applet.battstat"
...
#if 0
    /* FIXME: gde2-applets doesn't depend on libgde2 anymore */
    gde2_triggers_do ("", NULL, "battstat_applet", "LowBattery", NULL);
#endif
  }
...
#if 0
    /* FIXME: gde2-applets doesn't depend on libgde2 anymore */
    gde2_triggers_do ("", NULL, "battstat_applet", "BatteryFull", NULL);
#endif
...
battstat_show_help( ProgressData *battstat, const char *section )
{
  ...
  if (section)
    uri = g_strdup_printf ("help:gde2-battstat/%s", section);
  else
    uri = g_strdup ("help:gde2-battstat");
...
{
  if( battstat->orienttype == GDE2_PANEL_APPLET_ORIENT_RIGHT )
    gtk_label_set_angle( GTK_LABEL( battstat->percent ), 90 );
  else if( battstat->orienttype == GDE2_PANEL_APPLET_ORIENT_LEFT )
...
*/
static void
change_orient (Gde2PanelApplet       *applet,
...
*/
static void
size_allocate( Gde2PanelApplet *applet, GtkAllocation *allocation,
               ProgressData *battstat)
{
...
{
  Gde2PanelApplet *applet = GDE2_PANEL_APPLET (battstat->applet);
...
create_layout(ProgressData *battstat)
{
  ...
  gde2_panel_applet_set_background_widget( GDE2_PANEL_APPLET( battstat->applet ),
 ...
 */
static gboolean
battstat_applet_fill (Gde2PanelApplet *applet)
{
  ProgressData *battstat;
  ...
  battstat->settings = gde2_panel_applet_settings_new (applet, BATTSTAT_SCHEMA);
  ...
  battstat->orienttype = gde2_panel_applet_get_orient (applet);
  ...
  gde2_panel_applet_setup_menu_from_file (GDE2_PANEL_APPLET (battstat->applet),
				     ui_path, action_group);
  g_free (ui_path);

  if (gde2_panel_applet_get_locked_down (GDE2_PANEL_APPLET (battstat->applet))) {
	  GtkAction *action;
...
/* Boilerplate... */
static gboolean
battstat_applet_factory (Gde2PanelApplet *applet,
			 const gchar          *iid,
			 gpointer              data)
{
...
GDE2_PANEL_APPLET_OUT_PROCESS_FACTORY ("BattstatAppletFactory",
				  PANEL_TYPE_APPLET,
				  "battstat",
